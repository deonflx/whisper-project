(function() {
  ...
  // GIF paths - you'll need to add these GIF files to your extension
  const gifPaths = {
  ...
  function extractSubtitles() {
    ...
    // Method 2: DOM subtitles (YouTube, Netflix)
  ...
  function displaySignGifs(tokens) {
    ...
    // Reset and start showing tokens
    currentTokenIndex = 0;
  ...
  function showToken(tokens, signGif, signText) {
    if (currentTokenIndex >= tokens.length) {
      currentTokenIndex = 0; // Loop back to start
      hideSignGifs();
      return;
    }
    ...
    if (gifPath) {
      ...
      // Show each token for 1.5 seconds
  ...
  window.addEventListener('beforeunload', () => {
    if (signGifOverlay) {
      signGifOverlay.remove();
    }
  });

  // Periodic video detection
  setInterval(() => {
    if (!currentVideo) {
      findVideoElements();
    }
